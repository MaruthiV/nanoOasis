...
**A from-scratch reference implementation of a diffusion world model.** The whole pipeline in ~2,400 lines
of readable Python, trainable end-to-end for **under $50**, and playable in your browser.
...
2. A small **ViT-VAE** compresses each 256×192 frame to a 16×24×32 latent. The grid is designed so **one
   game cell maps to exactly one DiT token** (an 8×6 board → 48 tokens), so nothing the model has to render
   is ever smaller than a token.
...
- **Diffusion Forcing** (Chen 2024): independent per-frame noise plus a causal temporal mask, for stable
